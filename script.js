...
function drawLabel(text, x, y, color) {
  ...
  ctx.fillStyle = color;
  ctx.fillRect(x, y - 16, tw + padX * 2, 16);
...
function render(detections) {
  ...
  detections.forEach(d => {
    ...
    const x1 = nx1 * overlay.width, y1 = ny1 * overlay.height;
    const x2 = nx2 * overlay.width, y2 = ny2 * overlay.height;
...
async function triggerFamilyCall(risk) {
  ...
  try {
    ...
    momentStatus.textContent = 'Family member called';
    logEvent('Emergency call placed through Twilio', 'CRITICAL');
  } catch (error) {
    momentStatus.textContent = 'Family call failed: configure Twilio';
...
function speakPapa() {
  ...
  const message = new SpeechSynthesisUtterance('Papa, please help.');
  message.rate = 0.9;
...
function captureBestMoment(risk, dataUrl) {
  ...
  logEvent(`Best alert moment captured — ${risk.object || 'object'} near person`, risk.label);
}
...
function applyRisk(risk, dataUrl) {
  ...
  if (risk.object) {
    ...
    riskDetail.textContent = 'No person + dangerous object both in frame';
  }

  if ((risk.label === 'HIGH RISK' || risk.label === 'CRITICAL') && risk.label !== lastLoggedLevel) {
    logEvent(`${risk.label} — ${risk.object || 'unknown object'} near person (score ${risk.score})`, risk.label);
  }
